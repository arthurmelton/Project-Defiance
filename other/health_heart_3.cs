...
namespace other
{
    public class health_heart_3 : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (_playerHealth.Health >= ((maxPlayerHealth / 5)) * 3)
            {
                _heart1.localScale = new Vector3(0.76f, 0.7875f);
            }

            if (_playerHealth.Health >= ((maxPlayerHealth / 5)) * 3) return;
            _healthThing = ((_playerHealth.Health - ((maxPlayerHealth / 5) * 2)) * 3.8);
